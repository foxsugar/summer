            ...
            if ((room.getMode().equals("1") || room.getMode().equals("3") || room.getMode().equals("11") || room.getMode().equals("13"))) {//平胡
                if (this.userId == this.gameInfo.firstTurn) {//庄赢
                    for (Long i : gameInfo.getPlayerCardsInfos().keySet()) {
                        gameInfo.getPlayerCardsInfos().get(i).setScore(gameInfo.getPlayerCardsInfos().get(i).getScore() - 4 * room.getMultiple() * gangkaiScala);
                        room.setUserSocre(i, -4 * room.getMultiple() * gangkaiScala);
                    }
                    this.score = this.score + 4 * room.getPersonNumber() * room.getMultiple() * gangkaiScala;
                    room.setUserSocre(this.userId, 4 * room.getPersonNumber() * room.getMultiple() * gangkaiScala);
                    this.fan = 2;
                } else {
                    for (Long i : gameInfo.getPlayerCardsInfos().keySet()) {
                        gameInfo.getPlayerCardsInfos().get(i).setScore(gameInfo.getPlayerCardsInfos().get(i).getScore() - 2 * room.getMultiple() * gangkaiScala);
                        room.setUserSocre(i, -2 * room.getMultiple() * gangkaiScala);
                    }
                    //庄输2倍，在此多减一倍
                    gameInfo.getPlayerCardsInfos().get(this.gameInfo.firstTurn).setScore(gameInfo.getPlayerCardsInfos().get(this.gameInfo.firstTurn).getScore() - 2 * room.getMultiple() * gangkaiScala);
                    room.setUserSocre(this.gameInfo.firstTurn, -2 * room.getMultiple() * gangkaiScala);

                    this.score = this.score + (room.getPersonNumber() + 1) * 2 * room.getMultiple() * gangkaiScala;
                    room.setUserSocre(this.userId, (room.getPersonNumber() + 1) * 2 * room.getMultiple() * gangkaiScala);
                    this.fan = 2;
                }
                    /*for (Long i : gameInfo.getPlayerCardsInfos().keySet()){
...
//        			this.winType.add(HuType.hu_普通胡);
                ...
                if (3 == MahjongCode.HUTOSCOREFORLQ.get("" + CardUtil.huForScores(cards, huCardType) + "")) {
                    ...
                    if (this.userId == this.gameInfo.firstTurn) {//庄赢
                        for (Long i : gameInfo.getPlayerCardsInfos().keySet()) {
                            gameInfo.getPlayerCardsInfos().get(i).setScore(gameInfo.getPlayerCardsInfos().get(i).getScore() - MahjongCode.HUTOSCOREFORLQ.get("" + CardUtil.huForScores(cards, huCardType)) * room.getMultiple() * 2 * gangkaiScala);
                            room.setUserSocre(i, -MahjongCode.HUTOSCOREFORLQ.get("" + CardUtil.huForScores(cards, huCardType)) * room.getMultiple() * 2 * gangkaiScala);
                        }
                        this.score = this.score + MahjongCode.HUTOSCOREFORLQ.get("" + CardUtil.huForScores(cards, huCardType)) * room.getPersonNumber() * room.getMultiple() * 2 * gangkaiScala;
                        room.setUserSocre(this.userId, MahjongCode.HUTOSCOREFORLQ.get("" + CardUtil.huForScores(cards, huCardType)) * room.getPersonNumber() * room.getMultiple() * 2 * gangkaiScala);
                        this.fan = MahjongCode.HUTOSCOREFORLQ.get("" + CardUtil.huForScores(cards, huCardType));
                    } else {
                        for (Long i : gameInfo.getPlayerCardsInfos().keySet()) {
                            gameInfo.getPlayerCardsInfos().get(i).setScore(gameInfo.getPlayerCardsInfos().get(i).getScore() - MahjongCode.HUTOSCOREFORLQ.get("" + CardUtil.huForScores(cards, huCardType)) * room.getMultiple() * gangkaiScala);
                        ...
                        room.setUserSocre(this.userId, (room.getPersonNumber() + 1) * MahjongCode.HUTOSCOREFORLQ.get("" + CardUtil.huForScores(cards, huCardType)) * room.getMultiple() * gangkaiScala);
                        this.fan = MahjongCode.HUTOSCOREFORLQ.get("" + CardUtil.huForScores(cards, huCardType));
                    }
						/*for (Long i : gameInfo.getPlayerCardsInfos().keySet()){
