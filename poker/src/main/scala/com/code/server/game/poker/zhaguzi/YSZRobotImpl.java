...
//                pass(game);
            ...
            //如果没在游戏中
            if (room.getCurGameNumber() > 1 && now - ((RoomYSZ) room).lastOverTime > 1000* 15) {
                logger.info("xxxxxxx:now{}:getLastReadyTime{}==inter:{}", now, ((RoomYSZ) room).getLastReadyTime(), (now - ((RoomYSZ) room).getLastReadyTime())/ 1000.0);
